...
<head>
    ...
    <link rel="stylesheet" href="garson.css">
</head>
<body>
    <header>
    ...
    <div class="container">
        ...
        <main class="main-content">
            ...
            $sonuc = $baglanti->executeQuery('kafedb.siparis', $sorgu);
            ...
            if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['onayla'])) {
                ...
                // sipariş koleksiyonundan veri çekme
                $sorgu = new MongoDB\Driver\Query(['_id' => new MongoDB\BSON\ObjectId($id)]);
                $sonuc = $baglanti->executeQuery('kafedb.siparis', $sorgu);
                $veri = $sonuc->toArray();

                if (!empty($veri)) {
                    // kayit koleksiyonuna veri ekleme
                    ...
                    $baglanti->executeBulkWrite('kafedb.kayit', $bulk);

                    // sipariş koleksiyonundan veri silme
                    $bulk = new MongoDB\Driver\BulkWrite;
